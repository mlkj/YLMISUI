...
export default {
    ...
    methods:{
           ...
           _currentChange(val){
                ...
                if(rows.length>0){
                    rows.map((item,index)=>{
                        ...
                        this.mapConfig[flag].map((model,index)=>{
                            if(model.isplace){
                                if(model.calculate===undefined){
                                     row[model.prop]=item[model.selprop];
                                }else{
                                    
                                       row[model.prop]=this.calculate(model.calculate,item);
                                }
                            }else{
                        ...
                        if(this.globConf.tableConf.limitCount>this.tableData.length){
            ...
            calculate(obj,item){
                if(obj.op==='+'){
                    return item[obj.coll]+item[obj.colr];
                }else if(obj.op==='-'){
                     return item[obj.coll]-item[obj.colr];
                }else if(obj.op==='*'){
                    return item[obj.coll]*item[obj.colr];
